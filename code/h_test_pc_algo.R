...
dependency_h_test <- function(trjs,variables, to,from,sep_set,alpha){
  cimsout_trjs = list()
  ...
  for(i in 1:nrow(comb)){
    ...
    for(x0 in 0:(variables[variables$Name==from,"Value"]-1)){
      ...
      PQ = cimsout[[to]][[index_value+x0*molt_var2+1]]
			...
			for(x in  1:variables[variables$Name==to, "Value"]){
				tmp_filtered = filter_trjs_by_values(filtered_samples, c(to, from), c(x-1, x0))
				r1 = 0
				for(s_index in seq(tmp_filtered))
					r1 = r1 + sum(as.matrix(tmp_filtered[[s_index]][paste(to,"_diff",sep="")]))
				theta_hat_frac = PQ0[x,x]/PQ[x,x]
				quantile = 1- alpha/2
				if(theta_hat_frac < qf(quantile,r0_values[[x]],r1, lower.tail=FALSE) || theta_hat_frac > qf(quantile, r0_values[[x]],r1))
					return(TRUE)
			}
    }
  }
...
h_test_pc_algo <- function(trjs,variables,alpha=0.05){
  vars = variables$Name
    ...
    for(to in setdiff(vars,exausted_vars)){
      ...
      for(from in from_variables){
        ...
        for(index_set in 1:nrow(sep_set_comb)){
          ...
          if(!dependency_h_test(trjs_with_diff, variables, to,from,sep_set,alpha)){
